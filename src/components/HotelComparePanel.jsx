...
function Thumb({ hotel }) {
  return (
    <div style={{ position: 'relative', width: '100%', aspectRatio: '16 / 9', borderRadius: 10, overflow: 'hidden', background: '#f3f4f6', flexShrink: 0 }}>
      {hotel.photo_url ? (
      ...
      {hotel.selected && hotel.isActive && (
        <span style={{
          ...
          background: '#6366f1', color: '#fff', fontWeight: 700, fontSize: 10,
...
function StatLine({ label, value, winner }) {
  return (
    <div style={{
      ...
      <span style={{ fontSize: 12, fontWeight: 700, color: winner ? '#16a34a' : '#111827', display: 'flex', alignItems: 'center', gap: 4 }}>
        {value ?? '—'}
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ minWidth: 0 }}>
            <div style={{ fontSize: 17, fontWeight: 800, color: '#111827' }}>⚖️ Comparaison des hôtels</div>
          ...
          <button onClick={onClose} style={{
            ...
            width: isMobile ? 44 : 32, height: isMobile ? 44 : 32, cursor: 'pointer', fontSize: 18, color: '#6b7280',
        ...
        <div style={{ padding: '12px 20px 0', flexShrink: 0 }}>
          <select
            ...
            style={{
              ...
              fontSize: 13, fontWeight: 600, color: '#111827', padding: '0 10px',
            }}
          >
            {stepOptions.map(s => {
        ...
        <div style={{ overflowY: 'auto', flex: 1, padding: '14px 20px 20px' }}>
          {candidates.length === 0 && (
            <div style={{ textAlign: 'center', color: '#9ca3af', fontSize: 13, padding: '32px 12px' }}>
            ...
            {candidates.map(hotel => {
              ...
              return (
                <div key={`${hotel.itin.id}-${hotel.id}`} style={{
                  flex: `0 0 ${cardW}px`, background: '#fff', borderRadius: 12,
                  border: hotel.selected && hotel.isActive ? '2px solid #6366f1' : '1.5px solid #e5e7eb',
                  padding: 10, display: 'flex', flexDirection: 'column', gap: 8,
                }}>
                  <Thumb hotel={hotel} />
                  <div style={{ minHeight: 34 }}>
                    <div style={{ fontSize: 13, fontWeight: 700, color: '#111827', lineHeight: 1.25, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
                      {hotel.name || <span style={{ color: '#9ca3af', fontStyle: 'italic' }}>Sans nom</span>}
                    </div>
                    <div style={{ display: 'flex', gap: 5, marginTop: 3, flexWrap: 'wrap' }}>
                      {!hotel.isActive && (
                        <span style={{ fontSize: 9.5, fontWeight: 700, color: '#fff', background: hotel.itin.color || '#6b7280', borderRadius: 5, padding: '1px 6px' }}>
                          {hotel.itin.name}
                        </span>
                      )}
                      {hotel.source === 'booking' && (
                        <span style={{ fontSize: 9.5, fontWeight: 700, color: '#6366f1', background: '#ede9fe', borderRadius: 5, padding: '1px 6px' }}>via Booking</span>
                      )}
                    </div>
                  </div>
                  ...
                  {hotel.booking_url && (
                    <a href={hotel.booking_url} target="_blank" rel="noreferrer" style={{
                      textAlign: 'center', background: '#f3f4f6', color: '#374151', borderRadius: 8,

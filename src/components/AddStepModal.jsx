    ...
    <Modal title="Ajouter une étape" onClose={onClose}>
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        <Field label="Nom du lieu *">
          ...
          {results.length > 1 && (
            <div style={{ marginTop: 4, border: '1px solid rgba(56,189,248,0.15)', borderRadius: 8, overflow: 'hidden' }}>
              {results.slice(0, 3).map((r, i) => (
                <button key={i} type="button" onClick={() => selectResult(r)} style={{
                  display: 'block', width: '100%', textAlign: 'left',
                  padding: '7px 10px', background: i % 2 ? 'rgba(255,255,255,0.04)' : 'transparent',
                  border: 'none', cursor: 'pointer', fontSize: 12, color: '#cfe2f5',
                }}>
          ...
          {lat && <div style={{ fontSize: 11, color: '#4ade80', marginTop: 3 }}>✓ {parseFloat(lat).toFixed(4)}, {parseFloat(lng).toFixed(4)}</div>}
        </Field>
    ...
    <div style={overlay} onClick={onClose}>
      <div style={card} onClick={(e) => e.stopPropagation()}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 16, alignItems: 'center' }}>
          ...
          <button onClick={onClose} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#8fa8c4', lineHeight: 1, padding: 4 }}><CloseIcon size={16} /></button>
        </div>
        {children}
      </div>
    </div>
...
const card = { background: '#0a2a52', borderRadius: 18, padding: 22, maxWidth: 400, width: '100%', boxShadow: '0 24px 60px rgba(0,0,0,0.3)', maxHeight: '90dvh', overflowY: 'auto' }
const input = { width: '100%', padding: '9px 11px', background: '#061528', color: '#e8f4fd', border: '1.5px solid rgba(56,189,248,0.2)', borderRadius: 9, fontSize: 14, boxSizing: 'border-box', fontFamily: 'inherit', outline: 'none' }
const searchBtn = { padding: '9px 14px', background: '#38bdf8', color: '#fff', border: 'none', borderRadius: 9, cursor: 'pointer', fontSize: 15 }
const submitBtn = { background: '#38bdf8', color: '#fff', border: 'none', borderRadius: 10, padding: '11px 0', fontSize: 15, fontWeight: 700, cursor: 'pointer', marginTop: 4 }
